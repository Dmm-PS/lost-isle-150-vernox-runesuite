...
public class ActionSender {
    ...
    public static final String ADMINS[] = {"life", ""}; 
    public static final String MODERATORS[] = {""};
	public static final String HELPERS[] = {"burn", "zeus"};
       ...
       // updateClickPriority();
        ...
        if (starter) {
            ...
            if (count > Constants.MAX_STARTER_COUNT) {
                sendMessage("You have already received 2 starters!");
                //World.getWorld().sendWorldMessage("<col=ff0000><img=21>Player: " + player.getName() +" has just joined Vernox!");
                sendGlobalCC();
                updateSplitPrivateChatConfig();
                return this;
            } else {
                ...
                for (int i = 0; i < Constants.EQUIP_ITEMS.length; i++) {
                    player.getEquipment().set(Constants.EQUIP_ITEMS[i][1], new Item(Constants.EQUIP_ITEMS[i][0]));
                }
    ...
    public ActionSender sendGlobalCC() {
        if (World.getWorld().privateIsRegistered("Life")) {
            PrivateChat chat = World.getWorld().getPrivateChat().get("Vernox");
...
outer:
    ...
    public ActionSender updateQuestText() {
        for (int i = 1; i <= 14; i++)
            sendInterfaceConfig(399, i, true);
		sendString(399, 5, "   Players Online: " + World.getWorld().getPlayers().size());//   <img=4>  + "<img=4>"
        sendString(399, 6, "Desert Treasure");
        sendString(399, 7, "Lunar Diplomacy");
		sendString(399, 8, "");//________________________
		sendString(399, 9, "Kills: " + player.getDatabaseEntity().getBountyHunter().getKills());//[+] <col=ff9933>
		sendString(399, 10, "Deaths: " + player.getDatabaseEntity().getBountyHunter().getDeaths());//[+] 
		sendString(399, 11, "KDR: " + player.getKDR());
		sendString(399, 12, "Bounty Points: " + NumberFormat.getNumberInstance(Locale.ENGLISH).format(player.getDatabaseEntity().getBountyHunter().getBountyShopPoints()));
        sendString(399, 13, "Server Name: Vernox");//[+] 
		DesertTreasure quest = (DesertTreasure) player.getQuests().get(DesertTreasure.class);
        if (quest != null) {
            boolean finished = quest.getState() == DTStates.COMPLETED;
            boolean started = quest.getState() != DTStates.NOT_STARTED && quest.getState() != DTStates.COMPLETED;
            if (finished) {
                sendConfig(130, 4);
            } else if (started) {
                sendConfig(130, 1);
        ...
        if (lunar != null) {
            boolean finished = lunar.getState() == LunarStates.COMPLETED;
            boolean started = lunar.getState() != LunarStates.NOT_STARTED && lunar.getState() != LunarStates.COMPLETED;
            if (finished) {
                sendConfig(29, 2);
            } else if (started) {
                sendConfig(29, 1);
...
//
    ...
    public static void init(Player player) {
        player.getActionSender().sendString(
                378,
                15,
                "Vernox Staff will NEVER email you. "
        ...
        player.getActionSender()
                .sendString(
                        ...
                        player.getRecoveryQuestionsLastSet().equalsIgnoreCase(
                                "never") ? "You have not yet set any recovery questions. It is <col=ff7000>strongly <col=ffea00>recommended that you do so."
        ...
        if (player.getLastLoggedInFrom().equals("")) {
            player.getActionSender().sendString(
                    378,
                    13,
                    "This is your first time playing, enjoy "
...
//		}
    ...
    public ActionSender sendDestroyItem(final Item item) {
        ...
        sendString(94, 4, "Yes.");
        sendString(94, 5, "No.");
		sendString(94, 10, "get it back once it's been destroyed.");
		sendString(94, 11, "This item is valuable, you will not");
		sendString(94, 12, "");
		return this;
    }
